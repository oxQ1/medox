...
async def ban_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        msg_text = "**ارفعني مسرف بجميع صلاحيات لكي استطيع طرد‌‌**"
        return msg_text, False
    except UserAdminInvalid:
        msg_text = "**لا أستطيع إزالة المشرف بجدية؟😂🙂‌‌**"
        return msg_text, False
    except Exception as e:
        if user_id in SUDOERS:
            msg_text = "**لا استطيع طرد مطور**"
            return msg_text, False
        
        msg_text = f"**لماذا تريدني أن أطرد نفسي؟**"
        return msg_text, False
...
async def unban_user(user_id, first_name, admin_id, admin_name, chat_id):
    try:
        ...
        msg_text = "**ارفعني مسرف بجميع صلاحيات لكي استطيع ازله •**"
        return msg_text
    except Exception as e:
        msg_text = f"**لماذا تريدني أن ازلة نفسي؟**"
        return msg_text

    user_mention = mention(user_id, first_name)
    admin_mention = mention(admin_id, admin_name)
    
    msg_text = f"**تم ازلتو من القايمه {user_mention}\nبوسطة: {admin_mention}**"
    return msg_text
...
@app.on_message(filters.command(["طرد"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]))
async def ban_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "**عذرا هذا الامر يخص الادمن**"
    ...
    if len(message.command) > 1:
        if message.reply_to_message:
            ...
            try:
                ...
                if user_obj == None:
                    return await message.reply_text("**لا استطيع العثور علي الشخص•**")
                user_id = user_obj[0]
        ...
        await message.reply_text("**الرجاء إدخال اسم المستخدم مع الأمر أو الرد على رسالة ذلك المستخدم🖤•‌‌**")
        return
...
@app.on_message(filters.command(["الغاء الطرد"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]))
async def unban_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "**عذراهذاالامريخص الادمن**"
            return await message.reply_text(msg_text)
    else:
        msg_text = "**عذرا هذاالامر يخص الادمن**"
        return await message.reply_text(msg_text)
    ...
    if len(message.command) > 1:
        try:
            ...
            user_obj = await get_userid_from_username(message.command[1])
            if user_obj == None:
                    return await message.reply_text("**عذرا هذاالامر يخص الادمن🖤•**")
            user_id = user_obj[0]
            first_name = user_obj[1]
